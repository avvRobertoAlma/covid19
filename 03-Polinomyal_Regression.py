...
_degree = int(sys.argv[3]) if len(sys.argv) >= 4 and sys.argv[3] else 5
...
regione = [x[_filtro] for x in andamento if x["denominazione_regione"] == _regione]
...
to_predict_x = [len(regione)+i for i in range(7)]
...
plt.title(f'Predizione per la regione: {_regione}')  
plt.xlabel('giorni')  
